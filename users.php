<?php
	...
	class User
	{
		...
		//added admin marker		
		public $is_admin;
		public $imageLocation = null;//make null by default to easly see if it exists
		public $isOnline;
		
		/*
	 	* Creates a new user.
	 	*/
		public function __construct($PID, $firstName, $lastName, $userName, $gender, $mobileNumber, $email, $password, $question_id, $question_answer, $is_admin) {
			$saltValue = "ct310";
			if(isset($PID)) {
				$this->PID = $PID;
			}		
	...
	public static function getUserFromRow($row){
		...
		$user->question_answer = $row['question_answer'];
		...
		public static function getUser($username, $password){
			...
			if($array->username == $username){
				if($array->password == crypt($password, $saltValue)){
					return $array;
				}
			}
			return null;
